...
# Parquet fast-path (May 2026): generated by scripts/build_airport_parquet.py
# Loaded ~10x faster than CSV; CSV remains the source of truth.
_AIRPORTS_PARQUET = _DATA_DIR / "airports_full.parquet"
_RUNWAYS_PARQUET = _DATA_DIR / "runways.parquet"
...
def load_airports() -> pd.DataFrame:
    ...
    """
    if _AIRPORTS_PARQUET.exists():
        df = pd.read_parquet(_AIRPORTS_PARQUET)
        # Parquet already has display_name + filtering applied.
        return df
...
def _load_runways_raw() -> pd.DataFrame:
    ...
    if _RUNWAYS_PARQUET.exists():
        return pd.read_parquet(_RUNWAYS_PARQUET)
